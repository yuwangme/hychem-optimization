...
def init_x(init_type):
    if init_type == "shell":
        ...
        x = hc.params_to_x(np.array([0.1, 0.7, 0.1, 0.5, 0.5, 0.5,
                                     ...
                                     3.61E+01, 3.92, 727.4,
                                     4.49E+12, 0.00, 26983.7]),
                           hc.A2_C1_BOUNDS)
    elif init_type == "prev":
        # initialization of x (best x from a previous train)
        x = np.array(
            [1.19E+00, 2.87E+00, -3.69E+00, -1.98E+00, -1.40E+00, 7.90E-01,
...
def hychem_sgd(
    ...
    for epoch in range(epoch_max):
        ...
        for iter in range(len(order)):
            ...
            cost = cost+cost_i
            print("\titer", iter, Tlist[i], "K",
                  "\ts(ci)", "{:.3e}".format(np.sqrt(cost_i)),
                  "\tdeltaL1 {:.3e}".format(np.linalg.norm(delta, 1)))
        costs[epoch] = cost/N_td
        if epoch == 0:
            cost0 = costs[epoch]
        print("epoch", epoch,
              "\tcost", "{:.3f}".format(np.sqrt(costs[epoch]/cost0)))
    end_time = time.time()
...
def plot_iter(x_per_iter, idx_range, costs):
    ...
    plt.figure()
    plt.plot(range(x_per_iter.shape[0]),
             x_per_iter[:, idx_range[idx_range <= 5]], '--')
    plt.plot(range(x_per_iter.shape[0]),
             x_per_iter[:, idx_range[idx_range > 5]])
    plt.legend(labels=[names[i] for i in idx_range],
    ...
    np.savetxt("../debug_figures/x_per_iter.csv", x_per_iter[:, idx_range],
               delimiter=",", comments='',
               header="".join([names[i]+"," for i in idx_range])[:-1])
